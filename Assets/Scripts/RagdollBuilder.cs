...
/// kinematic/disabled (while alive, animator-driven) and dynamic (on death). This tool
/// builds them once per character. Select the character's root GameObject (the one with
/// the Humanoid Animator - the Player rig, or an enemy prefab instance) and run
/// Tools > FPS Game > Build Ragdoll. Works off Animator.GetBoneTransform (HumanBodyBones),
/// so it works on any humanoid-retargeted rig using this project's model, not just one
/// specific FBX's bone names.
///
/// Safe to re-run: any bone that already has a Rigidbody is left completely alone, so
/// re-running after tweaking something else won't duplicate components. To pick up changed
/// joint limit VALUES on a rig that's already been built, delete its existing CharacterJoint
/// components first (Rigidbody/Collider can stay) and re-run - Pass 2 below only sets limits
/// on joints it creates.
///
...
/// axis) with a twist limit for the bone's own internal rotation and swing1/2 for its main
/// bend + sideways give. Knees and elbows are the exception - a real knee/elbow only bends
/// one direction (0 degrees to ~140 degrees), which CharacterJoint's swing limits can't
/// express (swing1Limit/swing2Limit are always symmetric +/-limit). So for those two bones
/// only, 'axis' is pointed down the BEND direction (local Z) instead of the chain direction,
/// and the (asymmetric) twist limit is used to enforce "0 to ~140, forward only" - swing1/2
/// then cover the two directions that should barely move at all (no sideways knee wobble, no
/// internal shin twist). Values are a reasonable starting point, not measured against this
/// project's actual rig - tune by eye in Play Mode with a ragdoll death, like everything else
/// placement/feel-related here.
/// </summary>
public static class RagdollBuilder
{
    ...
    // Mass is computed from each built capsule's actual volume (see BoneDensity) rather than a
    // fixed number, so it adapts to whatever scale/proportions a given model uses.
    ...
    // kg per cubic metre of capsule volume. Not meant to be biologically accurate - just a
    // constant that keeps mass proportional to size for whatever model this runs on.
    ...
    {
        // Some rigs have a non-humanoid Animator sitting on a parent object above the actual
        // body rig's Animator - just taking GetComponent<Animator>() on the selected object can
        // find that one first and wrongly report "no Humanoid Animator" even though the real one
        // is right there in a child. Search every Animator in the hierarchy and use the first
        // Humanoid one.
        Animator anim = null;
        ...
        // Hips branches three ways (spine + two legs) so it has no single well-defined "chain
        // length" the way every other bone here does via its own child - size its radius off
        // the whole torso+head span instead.
        ...
        foreach (var spec in Bones)
        {
            ...
            // Projection is a direct position SNAP applied outside the normal physics step - it
            // bypasses collision detection, which can teleport a bone (and everything downstream
            // of it) through floor/wall geometry in one frame. A joint stretching slightly for a
            // frame while the solver catches up is far preferable to a silent teleport.
